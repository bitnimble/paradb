...
const config = {
  // Only files with a `.test`/`.spec` suffix are suites; plain modules under `tests/` (e.g.
  // map_generator.ts) are helpers, not tests.
  testMatch: ['**/tests/**/*.+(spec|test).[jt]s?(x)', '**/?(*.)+(spec|test).[jt]s?(x)'],
